    ...
    <div className="bg-white rounded-xl shadow-lg border border-slate-100 overflow-hidden max-w-3xl w-full mx-auto animate-fade-in-up">
      ...
      <div className="p-6 md:p-8">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-8 text-slate-700">
          <div className="flex-1 p-4 bg-slate-50 rounded-lg border border-slate-200 w-full md:w-auto">
            <h3 className="font-bold text-slate-900 mb-1 text-lg">{dimension.leftLabel}</h3>
          ...
          <div className="flex-1 p-4 bg-slate-50 rounded-lg border border-slate-200 w-full md:w-auto text-right md:text-left">
            <h3 className="font-bold text-slate-900 mb-1 text-lg">{dimension.rightLabel}</h3>
            ...
            <div className="grid grid-cols-5 gap-2 sm:gap-4 max-w-xl mx-auto">
                {[1, 2, 3, 4, 5].map((num) => (
                    <button
                        ...
                        className="aspect-square rounded-lg border-2 border-slate-200 hover:border-slate-800 hover:bg-slate-50 text-slate-600 hover:text-slate-900 font-bold text-lg sm:text-xl transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-slate-500 flex items-center justify-center shadow-sm"
